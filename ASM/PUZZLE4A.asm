...
* PUZZLE4A mainline                                                   *
*---------------------------------------------------------------------*
PUZZLE4A CEEENTRY AUTO=WORKDSA_SIZ,MAIN=YES,BASE=(R10,R11)
*
...
*                                  * Was memory successfully allocated?
         IF (CLC,FC(8),NE,=XL8'0000000000000000') THEN
            ...
            B     PUZZLE4A_RET_IMMED * End program
         ENDIF
*
...
*
         ...
         BNE   PUZZLE4A_RET        * If not, get out
*
READNEXT EQU   *
*
...
*
PUZZLE4A_RET EQU   *               * Branch here for normal end
*
...
*
PUZZLE4A_RET_IMMED EQU   *         * Branch here when alloc mem failed
*
...
*
         MVC   OVERLAPCOUNT,=A(0)  * Initialize overlap counter
*
         MVC   PATHLEN,=A(27)
         MVC   PATHNAME(27),=C'/u/yin/ybtks/day4_input.txt'
...
*
         IF (NZ) THEN
*
            LA    R6,INPREC        * Start at beginning of rec
*
            DO FROM=(R14,4)        * Loop 4 times
*
...
*
               ...
               LA    R15,4         * Calculate offset to R1LOW
               SR    R15,R14       *    by getting 4-R14
               SLL   R15,1         *    and multiplying by 2
               LA    R1,R1LOW      * Then point to R1LOW
               AR    R1,R15        *    and add offset
               STH   R5,0(,R1)     * Store number in range
*
               IF (C,R14,GT,=A(1)) THEN * Skip non-numbers, except at
                  XR    R5,R5      *   the end
...
*
            ENDDO
*
*           If either range 1 envelopes range 2
            IF (CLC,R1LOW,LE,R2LOW),AND,                               X
               (CLC,R1HIGH,GE,R2HIGH) THEN
               ASI   OVERLAPCOUNT,1 * Increase overlap counter
*
*           Or if range 2 envelopes range 1
            ELSEIF (CLC,R2LOW,LE,R1LOW),AND,                           X
               (CLC,R2HIGH,GE,R1HIGH) THEN
               ASI   OVERLAPCOUNT,1 * Increate overlap counter
            ENDIF
*
         ENDIF
*
...
*
         L     R15,OVERLAPCOUNT    * Put overlap counter in R15
...
*                                  * Error messages
                             DS    0F
RESULTMSG                    DC    C'COMPLETE OVERLAP COUNT ',X'00'
                             DS    0F
