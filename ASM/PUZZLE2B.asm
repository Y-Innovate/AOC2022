...
* PUZZLE2B mainline                                                   *
*---------------------------------------------------------------------*
PUZZLE2B CEEENTRY AUTO=WORKDSA_SIZ,MAIN=YES,BASE=(R10,R11)
*
...
*                                  * Was memory successfully allocated?
         IF (CLC,FC(8),NE,=XL8'0000000000000000') THEN
            ...
            B     PUZZLE2B_RET_IMMED * End program
         ENDIF
*
...
*
         ...
         BNE   PUZZLE2B_RET        * If not, get out
*
READNEXT EQU   *
*
...
*
PUZZLE2B_RET EQU   *               * Branch here for normal end
*
...
*
PUZZLE2B_RET_IMMED EQU   *         * Branch here when alloc mem failed
*
...
*
         IF (CLC,INPRECLEN,EQ,=A(3)) THEN * Just to be sure skip empty
            ...
            WHEN C'A'              * They chose A = Rock
               SELECT CLI,2(R7),EQ * Determine desired outcome
               WHEN C'X'           * I should lose,so I choose Scissors
                  LA    R6,3       * 3 for Scissors, loss = no extra
               WHEN C'Y'           * We should draw, so I choose Rock
                  LA    R6,1+3     * 1 for Rock plus 3 for draw
               WHEN C'Z'           * I should win, so I choose Paper
                  LA    R6,2+6     * 2 for Paper plus 6 for win
               ENDSEL
            WHEN C'B'              * They chose B = Paper
               SELECT CLI,2(R7),EQ * Determine desired outcome
               WHEN C'X'           * I should lose, so I choose Rock
                  LA    R6,1       * 1 for Rock, loss = no extra
               WHEN C'Y'           * We should draw, so I choose Paper
                  LA    R6,2+3     * 2 for Paper plus 3 for draw
               WHEN C'Z'           * I should win, so I choose Scissors
                  LA    R6,3+6     * 3 for Scissors plus 6 for win
               ENDSEL
            WHEN C'C'              * They chose C = Scissors
               SELECT CLI,2(R7),EQ * Determine desired outcome
               WHEN C'X'           * I should lose, so I choose Paper
                  LA    R6,2       * 2 for Paper, loss = no extra
               WHEN C'Y'           * We should draw, I choose Scissors
                  LA    R6,3+3     * 3 for Scissors plus 3 for draw
               WHEN C'Z'           * I should win, so I choose Rock
                  LA    R6,1+6     * 1 for Rock plus 6 for win
               ENDSEL
            ENDSEL
*
