...
* PUZZLE5B mainline                                                   *
*---------------------------------------------------------------------*
PUZZLE5B CEEENTRY AUTO=WORKDSA_SIZ,MAIN=YES,BASE=(R10,R11)
*
...
*                                  * Was memory successfully allocated?
         IF (CLC,FC(8),NE,=XL8'0000000000000000') THEN
            ...
            B     PUZZLE5B_RET_IMMED * End program
         ENDIF
*
...
*
         ...
         BNE   PUZZLE5B_RET        * If not, get out
*
READNEXT EQU   *
*
...
*
PUZZLE5B_RET EQU   *               * Branch here for normal end
*
...
*
PUZZLE5B_RET_IMMED EQU   *         * Branch here when alloc mem failed
*
...
*
            ...
            LA    R3,2(R14,R3)     * Point R3 past last crate in stack
            S     R3,HOWMANY       * And walk back to first crate to mv
*
...
*
            ...
            B     *+10             * Skip past MVC instruction
            MVC   0(1,R2),0(R3)    * MVC instruction to EXecute
            EX    R1,*-6           * Move string
*
            MVI   0(R3),X'00'      * Clear out first crate moved
