...
* PUZZLE3A mainline                                                   *
*---------------------------------------------------------------------*
PUZZLE3A CEEENTRY AUTO=WORKDSA_SIZ,MAIN=YES,BASE=(R10,R11)
*
...
*                                  * Was memory successfully allocated?
         IF (CLC,FC(8),NE,=XL8'0000000000000000') THEN
            ...
            B     PUZZLE3A_RET_IMMED * End program
         ENDIF
*
...
*
         ...
         BNE   PUZZLE3A_RET        * If not, get out
*
READNEXT EQU   *
*
...
*
PUZZLE3A_RET EQU   *               * Branch here for normal end
*
...
*
PUZZLE3A_RET_IMMED EQU   *         * Branch here when alloc mem failed
*
...
*
         IF (NZ) THEN              * Just to be sure skip empty
            ...
            LA    R3,256           *    work TR table
            XR    R0,R0            *       with
            XR    R1,R1            *          all
            MVCL  R2,R0            *             zeros
*
            SRL   R7,1             * Divide length by 2
*
            LR    R6,R7            * Copy half length
            XR    R5,R5            * Clear out R5 for character
            XR    R4,R4            * Clear out R4 for offset
            LA    R3,INPREC        * Point R3 to input line
FILL_ENTRY  LA    R2,TR_WORK       * Point R2 to work TR table
...
*
            LR    R6,R7            * Copy half length again
            AR    R3,R6            * Point R3 to second half
            XR    R2,R2            * Prepare to receive TR result
            BCTR  R6,R0            * Prepare for EXecute
            B     *+10             * Skip past TR instruction to EX
            TRT   0(1,R3),TR_WORK  * Test for previously seen char
