...
* PUZZLE6B mainline                                                   *
*---------------------------------------------------------------------*
PUZZLE6B CEEENTRY AUTO=WORKDSA_SIZ,MAIN=YES,BASE=(R10,R11)
*
...
*                                  * Was memory successfully allocated?
         IF (CLC,FC(8),NE,=XL8'0000000000000000') THEN
            ...
            B     PUZZLE6B_RET_IMMED * End program
         ENDIF
*
...
*
         ...
         BNE   PUZZLE6B_RET        * If not, get out
*
READNEXT EQU   *
*
...
*
PUZZLE6B_RET EQU   *               * Branch here for normal end
*
...
*
PUZZLE6B_RET_IMMED EQU   *         * Branch here when alloc mem failed
*
...
READCHAR DS    0H
*
         MVC   INPREC(13),INPREC+1  * Shift last 13 read chars left 1
*
         LA    R7,INPREC+13        * Point R7 to pos 4
...
PROCESS  DS    0H
*
         IF (CLC,CHARCOUNT,LT,=A(14)) THEN * Skip until we have at
            B      PROCESS_RET     * least 14 chars
         ENDIF
*
         DO FROM=(R15,13)          * Loop 13 times
            LA    R14,13           * Get the current loop's offset by
            SR    R14,R15          *    subtracting loop count from 13
            LA    R2,INPREC        * Point R2 to start of string
            AR    R2,R14           * Advance to current offset
            LA    R3,INPREC+17     * Point R3 to start of str copy + 1
            AR    R3,R14           * Advance to current offset
            MVC   0(1,R3),0(R2)    * Copy single character at R2
            IF (C,R15,GT,=A(1)) THEN * If not at the last loop
               LR    R4,R15        * Copy loop count
               BCTR  R4,R0         * Decrease by 1, str copy 1 shorter
               BCTR  R4,R0         * Prepare for EXecute
               B     *+10          * Skip MVC to EXecute
               MVC   1(1,R3),0(R3) * Duplicate single char along str
               EX    R4,*-6        * EXecute MVC instruction
            ENDIF
            LR    R4,R15           * Copy loop count
            BCTR  R4,R0            * Prepare for EXecute
            B     *+10             * Skip XC to EXecute
            XC    0(1,R3),1(R2)    * Exclusive Or copy string
            EX    R4,*-6           * Execute XC instruction
*
            XR    R0,R0            * Search for zero (duplicate char)
            LA    R1,INPREC+17     * Point R1 to start of str copy + 1
            AR    R1,R14           * Advance to current offset
            LA    R2,INPREC+30     * Stop search at end of str copy
            SRST  R2,R1            * Scan for zero
            BRC   1,*-4            * Scan was incomplete, try again
            BRC   4,PROCESS_RET    * Zero was found, skip the rest
         ENDDO
*
